...
void pc_multi_sync_data(u32 info_ptr) {
  ...
  try {
    if (!gMultiplayerData.initialized || !gMultiplayerData.host)
      return;
    if (info_ptr == 0 || info_ptr < 0x1000)
      return;
      ...
      switch (event.type) {
        case ENET_EVENT_TYPE_RECEIVE: {
          if (event.packet->dataLength >= sizeof(PacketHeader)) {
            ...
            if (header->type == PacketType::STATE_UPDATE &&
              ...
              lg::info(
                  "[Multiplayer] Received Full Sync from Host: Money {}, Gems {}, Skill {}, Aids "
                  "{}",
        ...
        case ENET_EVENT_TYPE_CONNECT:
          ...
          if (gMultiplayerData.local_role == 0) {
            ...
            lg::info(
                "[Multiplayer] Sent Full Sync to new peer: Money {}, Gems {}, Skill {}, Aids {}",
    ...
    if (info->out_event_seq > gMultiplayerData.last_out_event_seq) {
      lg::info("[Multiplayer] Sending World Event: Type {}, AID {}, Seq {}", info->out_event_type,
               info->out_event_aid, info->out_event_seq);
      ...
      ENetPacket* event_packet =
          enet_packet_create(&out_event, sizeof(PacketWorldEvent), ENET_PACKET_FLAG_RELIABLE);
      if (gMultiplayerData.local_role == 0) {
